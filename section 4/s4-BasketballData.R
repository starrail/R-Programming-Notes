...
#return the row with row name and column names with drop=F
Games[1,,drop=F]
Games[1,5,drop=F]
...
# one player
Data <- MinutesPlayed[1,,drop=F] #drop=F: vector -> matrix
matplot(t(Data), type="b", pch=15:18, col=c(1:4,6))
legend("bottomleft", inset=0.01, legend=Players[1], col=c(1:4,6), 
       pch=15:18, horiz=F,cex=0.4)
...
myplot <- function(data, rows=1:10){
  ...
  legend("bottomleft", inset=0.01, legend=Players[rows],
         col=c(1:4,6), pch=15:18, horiz=F, cex=0.4)
}
